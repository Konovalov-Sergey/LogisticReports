...
const ReportOutDay: React.FC<PropsType> = () => {
    ...
    const handleClick = (warehouse: string, startDate: Date | null, endDate: Date | null) => {
        ...
        dispatch<any>(requestReportOutDay(warehouse, dateFrom, dateTo ))
    }

    const rows: GridRowsProp = reportOutDay.map(line => {
    ...
    const downloadExcel = () => {
        const workSheet = XLSX.utils.json_to_sheet(reportOutDay)
        const workBook = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(workBook, workSheet, "reportOutDay")
        //Buffer
        //let buf = XLSX.write(workBook, { bookType: "xlsx", type: "buffer" })
        //Binary string
        XLSX.write(workBook, { bookType: "xlsx", type: "binary" })
        //Download
        XLSX.writeFile(workBook, "reportOutDayData.xlsx")
    }
