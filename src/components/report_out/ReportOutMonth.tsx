...
const ReportOutMonth: React.FC<PropsType> = () => {
    ...
    const handleClick = (warehouse: string, startDate: Date | null, endDate: Date | null) => {
        ...
        dispatch<any>(requestReportOutMonth(warehouse, dateFrom, dateTo ))
    }

    const rows: GridRowsProp = reportOutMonth.map(line => {
        return { ...line, id: uuidv4() }
    });

    const columns: GridColDef[] = [
        { field: 'Рік', headerName: 'Рік', width: 80 },
        { field: 'Місяць', headerName: 'Місяць', width: 80 },
        { field: 'Склад', headerName: 'Склад', width: 80 },
        { field: 'Рейс', headerName: 'Рейс', width: 80 },
    ...
    const downloadExcel = () => {
        const workSheet = XLSX.utils.json_to_sheet(reportOutMonth)
        const workBook = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(workBook, workSheet, "reportOutMonth")
        //Buffer
        //let buf = XLSX.write(workBook, { bookType: "xlsx", type: "buffer" })
        //Binary string
        XLSX.write(workBook, { bookType: "xlsx", type: "binary" })
        //Download
        XLSX.writeFile(workBook, "reportOutMonthData.xlsx")
    }
        ...
        <div>
            ...
            {reportOutMonth.length > 0 &&
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ height: 500, width: '60%', marginTop: '20px' }}>
