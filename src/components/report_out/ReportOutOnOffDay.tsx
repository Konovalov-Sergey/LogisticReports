...
import { getReportOutOnOffDay } from '../../Redux/reportOut-selector';
import { requestReportOutOnOffDay } from '../../Redux/reportOut-reducer';
...
const ReportOutOnOffDay: React.FC<PropsType> = () => {
    ...
    const handleClick = (warehouse: string, startDate: Date | null, endDate: Date | null) => {
        ...
        dispatch<any>(requestReportOutOnOffDay(warehouse, dateFrom, dateTo ))
    }

    const rows: GridRowsProp = reportOutOnOffDay.map(line => {
        return { ...line, id: uuidv4() }
    });

    const columns: GridColDef[] = [
        { field: 'Дата', headerName: 'Дата', width: 100 },
        { field: 'Склад', headerName: 'Склад', width: 90 },
    ...
    const downloadExcel = () => {
        const workSheet = XLSX.utils.json_to_sheet(reportOutOnOffDay)
        const workBook = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(workBook, workSheet, "reportOutOnOffDay")
        //Buffer
        //let buf = XLSX.write(workBook, { bookType: "xlsx", type: "buffer" })
        //Binary string
        XLSX.write(workBook, { bookType: "xlsx", type: "binary" })
        //Download
        XLSX.writeFile(workBook, "reportOutOnOffDayData.xlsx")
    }
