...
const ReportInMonth: React.FC<PropsType> = () => {
    ...
    const handleClick = (warehouse: string, startDate: Date | null, endDate: Date | null) => {
        ...
        dispatch<any>(requestReportInMonth(warehouse, dateFrom, dateTo ))
    }

    const rows: GridRowsProp = reportInMonth.map(line => {
        return { ...line, id: uuidv4() }
    });

    const columns: GridColDef[] = [
        { field: 'Рік', headerName: 'Рік', width: 100 },
        { field: 'Місяць', headerName: 'Місяць', width: 100 },
    ...
    const downloadExcel = () => {
        const workSheet = XLSX.utils.json_to_sheet(reportInMonth)
        const workBook = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(workBook, workSheet, "reportInMonth")
        //Buffer
        //let buf = XLSX.write(workBook, { bookType: "xlsx", type: "buffer" })
        //Binary string
        XLSX.write(workBook, { bookType: "xlsx", type: "binary" })
        //Download
        XLSX.writeFile(workBook, "reportInMonthData.xlsx")
    }
